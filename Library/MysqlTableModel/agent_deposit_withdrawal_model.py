...
class AgentDepositWithdrawal(Base):
    __tablename__ = 'agent_deposit_withdrawal'
    __table_args__ = {'comment': '代理客户端存取款信息'}
    ...
    parent_id = Column(String(10, 'utf8mb4_0900_bin'), comment='父节点')
    ...
    deposit_withdraw_channel_name = Column(String(100, 'utf8mb4_0900_bin'), comment='存取款通道name')
    ...
    trade_currency_amount = Column(DECIMAL(20, 8), server_default=text("'0'"), comment='交易币种金额')
    ...
    deposit_withdraw_address = Column(String(50, 'utf8mb4_0900_bin'), comment='存取款地址（银行卡账号，电子钱包账户，虚拟币地址）')
    ...
    status = Column(INTEGER(11), server_default=text("'1'"), comment='状态 1待一审 2一审审核 3一审拒绝 4待二审 5二审审核 6二审拒绝  7待三审 8三审审核 9 三审拒绝 10 待出款 20待处理  21处理中   90 已关闭 96出款失败 97出款取消 98取消订单（申请人） 100失败 101成功')
    review_operation = Column(INTEGER(11), comment='审核操作，system_param user_withdraw_review_operation值')
    is_big_money = Column(CHAR(1, 'utf8mb4_0900_bin'), comment='是否大额出款;0-否，1-是')
    ...
    pay_third_url = Column(String(500, 'utf8mb4_0900_bin'), comment='存取三方支付URL')
    ...
    collect_info = Column(String(500, 'utf8mb4_0900_bin'), comment='提款收集信息')
    combined_recharge = Column(INTEGER(1), comment='合并充值标记（虚拟币小额多笔）1 是 0否')
    ...
    remaining_amount = Column(DECIMAL(20, 2), comment='提款后剩余金额')
    daily_free_count = Column(INTEGER(11), comment='单日免费提款总次数')
    daily_free_withdrawal_total_amount = Column(String(100, 'utf8mb4_0900_bin'), comment='单日免费提款总额-单位:币种代码')
