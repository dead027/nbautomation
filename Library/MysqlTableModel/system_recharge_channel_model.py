...
class SystemRechargeChannel(Base):
    __tablename__ = 'system_recharge_channel'
    __table_args__ = {'comment': '充值通道配置'}
    ...
    recharge_way_id = Column(BIGINT(20), comment='充值方式ID')
    ...
    recharge_min = Column(DECIMAL(16, 4), comment='充值最小值')
    recharge_max = Column(DECIMAL(16, 4), comment='充值最大值')
    use_scope = Column(String(512, 'utf8mb4_0900_bin'), comment='使用范围')
    weight = Column(INTEGER(11), comment='同类型权重')
    auth_num = Column(INTEGER(11), server_default=text("'0'"), comment='授权数量')
