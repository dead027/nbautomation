...
class DateUtil(object):
    ...
    def get_month_day_num(diff=0):
        now = DateUtil.get_current_time("-5")
    ...
    @staticmethod
    def get_current_time(timezone="-5"):
        """
        根据时区返回当前时间
        :return:
        """
        return arrow.now(f"GMT{timezone}")

    @staticmethod
    def get_timestamp_by_now(diff=-1, timezone="-5"):
        """
        获取当前日期前的时间，不包含小时分钟秒
    ...
    @staticmethod
    def get_date_by_now(date_type="日", diff=-1, timezone="-5"):
        """
        获取当前日期前的时间，不包含小时分钟秒
    ...
    @staticmethod
    def _get_local_timezone_to_utc_diff():
        """
        针对沙巴，将本地时区与utc时区进行转换
        @return:
        """
        return arrow.now().utcoffset().seconds / 3600

    @staticmethod
    def get_utc_search_time(diff=-1, is_end=False, rtn_type='秒'):
        """
        获取沙巴体育的查询时间字符串   本地时区转utc
        :param diff:之后传正值，之前传负值
        :param is_end:是否是一天的最后
        :param rtn_type: 返回数据类型  秒 ｜ 日
        :return:
        """
        hour_diff = DateUtil._get_local_timezone_to_utc_diff()
        direct = '+' if hour_diff >= 0 else ''
        timezone_str = f'{direct}{hour_diff}'
        now = DateUtil.get_current_time(timezone_str)
        ...
        if is_end:
            ...
            return now.shift(days=int(diff)).replace(hour=23, minute=59, second=59).shift(hours=-hour_diff).strftime(
                str_format)
        else:
            # return now.shift(days=int(diff)).replace(hour=0, minute=0, second=0).shift(hours=5).strftime(str_format)
            return now.shift(days=int(diff)).replace(hour=0, minute=0, second=0).shift(hours=-hour_diff).strftime(
                str_format)

    @staticmethod
    def convert_utc_time_to_local(utc_time):
        """
        utc时间转本地时区
        :param utc_time
        :return:
        ...
        # direct = '+' if hour_diff >= 0 else ''
        # timezone_str = f'{direct}{hour_diff}'
        return arrow.get(utc_time, 'YYYY-MM-DDTHH:mm:ss').shift(hours=hour_diff).strftime("%Y-%m-%dT%H:%M:%S")

    @staticmethod
    def get_day_range(date_type="月", diff=0, stop_diff=None, timezone="-5"):
        """
        获取年、月的起始和结束日期，不含小时分钟秒
        :param date_type: 年|月|周|日|秒，默认为月
        :param diff:之后传正值，之前传负值
        ...
        if date_type == "月":
            ...
            new_date = now.shift(seconds=int(diff))
            start = new_date.strftime("%Y-%m-%d %H:%M:%S")
            end = new_date.strftime("%Y-%m-%d %H:%M:%S")
        else:
            raise AssertionError("类型只能为年月，实际传参为： %s" % date_type)
        return start, end

    def get_md_day_range(self, date_type="月", diff=-1, timezone="-5"):
        """
    ...
    def str_to_timestamp(time_str, is_end=False, timezone="md"):
        ...
        if timezone == 'md':
            ...
            diff = int(timezone[1:]) * 3600000
            diff = -diff if timezone[0] == "+" else diff
        timestamp = int(datetime.datetime.strptime(time_str, "%Y-%m-%d %H:%M:%S").replace(tzinfo=tz.utc).timestamp())
    ...
    @staticmethod
    def timestamp_to_date(timestamp, timezone='-5', exact_type='日', day_diff=0):
        """
        时间戳转日期
        :param timestamp:
        ...
        return arrow.get(timestamp).to(f'GMT{timezone}').shift(days=int(day_diff)).strftime(format_str)

    # @staticmethod
    # def timestamp_to_time(timestamp, timezone='-5'):
    #     """
    #     时间戳转日期
    #     :param timestamp:
    ...
    #     :return:
    #     """
    #     return arrow.get(timestamp).to(f'GMT{timezone}').strftime("%Y-%m-%d %H:%M:%S")

    @staticmethod
    def get_timestamp_range(start_diff=0, end_diff=0, stop_diff=None, date_type="日", timezone="-5"):
        ...
        if date_type in "年月周日":
            ...
            start_timestamp = DateUtil.str_to_timestamp(start_day, timezone=timezone)
            end_timestamp = DateUtil.str_to_timestamp(end_day, is_end=True, timezone=timezone)
...
if __name__ == '__main__':
    ...
    # print(result)
    # print(DateUtil.get_timestamp_range(0, 0, 0, '月', '+13'))
    print(DateUtil.get_current_time('+8'))
